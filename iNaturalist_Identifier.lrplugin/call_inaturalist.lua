...
local json = require("json") -- Ensure json.lua is included in your plugin

local M = {}

-- Identify species from an image using the iNaturalist API
function M.identify(imagePath, token)
    ...
    -- Construct multipart/form-data HTTP body
    ...
    -- Prepare HTTP headers with token and content type
    local headers = {
...
-- Submit an observation to iNaturalist using metadata from the photo
function M.submitObservation(photo, keywords, token)
    ...
    if not captureTime then
        captureTime = os.date("!%Y-%m-%dT%H:%M:%SZ")
    else
