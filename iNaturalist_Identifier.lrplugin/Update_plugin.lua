...
local function getLatestRelease()
	...
	logger.logMessage(
		"[Step 1] HTTP response: "
			.. tostring(respHeaders.status or "nil")
			.. ", error: "
...
local function downloadZip(url, zipPath)
	...
	logger.logMessage(
		"[Step 3] HTTP download response: "
			.. tostring(headers.status or "nil")
			.. ", error: "
...
local function downloadAndInstall(_ctx, release)
	...
	if not replacePlugin(pluginPath, extractedPlugin) then
		return
	end
